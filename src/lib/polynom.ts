...
export class Polynom {
  ...
  toMonomial(): Monomial {
    ...
    if (len === 0) return new Monomial(0, 0);
    if (len === 1) return this.monomials[0].copy();
  ...
  private addMon(mon: Monomial): void {
    const obj = this.monomials.find((element, _index, _array) => {
      if (element.isSameDegree(mon)) return true;
  ...
  mult(pol: Polynom): Polynom {
    ...
    for (let i = 0; i < lMons.length; i++) {
      for (let j = 0; j < rMons.length; j++) {
        res.addMon(lMons[i].mult(rMons[j]));
      }
    }
  ...
  toDegree(n: number): Polynom {
    if (n === 0) return new Polynom([new Monomial(1, 0)]);

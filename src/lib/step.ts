...
export function bellmanStep(
  ...
  for (let k = 1; k <= funct.xCoeff.length; k++) {
    const cf = xCoff.push(sum.find('x' + k) ?? 0);
...
function funcToExpr(funct: InFunctional): Expression {
  ...
  for (let i = num; i > 0; i--) {
    terms.unshift(new Term(funct.xCoeff[i - 1], name['x' + i]));
  }
 ...
 */
function addToExpr(expr: BellmanExpression, system: System): Expression {
...
function expendToTime(
  ...
  for (let k = 1; k <= n; k++) {
    terms.unshift(new Term(coeff * ext.x[k - 1], name['x' + k]));
  }

  terms.unshift(new Term(coeff * ext.u, name['u']));
}
